...
def sample_z_previous(real_X, index, npattern):  # type: ignore
...
def sample_z_later(pre, real_X, index, npattern):  # type: ignore
    Tensor = torch.FloatTensor
...
def sample_z_cn(real_X, npattern):  # type: ignore
    z_random = Variable(
...
def criterion_GAN(pred, target_is_real, prob):  # type: ignore
    if target_is_real:
...
def criterion_orthogonal(change_batch_sum, npattern):  # type: ignore
...
def mono_loss(later, prev):  # type: ignore
    return F.mse_loss(
...
class SurrealGAN(object):
    ...
    def create(self, opt):  # type: ignore
        ...
        self.phi = torch.nn.Parameter(
            torch.eye(self.opt.npattern)[tril_indices[0], tril_indices[1]]
        )
        self.initial_latent_distribution = guassian_colula_distribution(
            torch.eye(self.opt.npattern), self.opt.npattern
        )

        # definition of all optimizers
        self.optimizer_M = torch.optim.Adam(
            ichain(
                self.netMapping.parameters(),
                self.netDecomposer.parameters(),
                self.netReconstruction.parameters(),
            ),
            lr=self.opt.lr,
        ...
        self.optimizer_D = torch.optim.Adam(
            ichain(self.netDiscriminator.parameters()),
            lr=self.opt.lr / 5.0,
            betas=(self.opt.beta1, 0.999),
        )
        self.optimizer_phi = torch.optim.Adam(
            iter([self.phi]), lr=self.opt.lr / 30.0, betas=(self.opt.beta1, 0.999)
        )

    def train_instance(self, x, real_y, alpha):  # type: ignore
        z_pre = [
            sample_z_previous(x, i, self.opt.npattern) for i in range(self.opt.npattern)
        ]
        # z_pre_sum = torch.sum(torch.stack(z_pre), dim=0)
        z_later = [
            sample_z_later(z_pre[i], x, i, self.opt.npattern)
            for i in range(self.opt.npattern)
        ]
        # z_later_sum = torch.sum(torch.stack(z_later), dim=0)

        fake_y_pre_change = [
            self.netMapping.forward(x, z_pre[i]) for i in range(self.opt.npattern)
        ]
        fake_y_later_change = [
            self.netMapping.forward(x, z_later[i]) for i in range(self.opt.npattern)
        ]
        ...
        latent_distribution = guassian_colula_distribution(
            construct_scale_tril(self.phi, self.opt.npattern), self.opt.npattern
        )
        post_prob = torch.exp(
        ...
        loss_phi = self.criterionGAN(
            self.netDiscriminator.forward(fake_y.detach()).detach(), True, post_prob
        ...
        reconst_zt = [
            self.netReconstruction.forward(decomposed_fake_y[i])
            for i in range(self.opt.npattern)
        ]
        ...
        orthogonal_loss = self.criterionOrtho(
            fake_y_change_batch_sum, self.opt.npattern
        )
        ...
        loss_G = (
            ...
            + self.opt.lam * orthogonal_loss
            + self.opt.zeta * recons_loss
            + self.opt.kappa * decompose_loss
            + self.opt.gamma * change_loss
            + self.opt.mu * mono_loss
            + self.opt.eta * cn_loss
        )  # 7
    ...
    def decompose(self, real_y):  # type: ignore
    ...
    def predict_rindices(self, real_y):  # type: ignore
        decompose_y = self.netDecomposer.forward(real_y)
        reconst_zt_decompose = [
            self.netReconstruction.forward(decompose_y[i])
            for i in range(self.opt.npattern)
        ]
    ...
    def predict_Y(self, x, z):  # type: ignore
    ...
    # save checkpoint
    def save(self, save_dir, epoch, chk_name):  # type: ignore
    ...
    def load_opt(self, checkpoint):  # type: ignore
        self.opt = dotdict({})
    ...
    # load trained model
    def load(self, chk_path, epoch):  # type: ignore
        for i in range(100000):
            try:
